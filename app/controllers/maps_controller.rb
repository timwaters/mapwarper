class MapsController < ApplicationController
  ...
  def wms
    ...
    mapsv.setMetaData("wms_onlineresource",
      "#{request.scheme}://" + request.host_with_port + rel_url_root + "/maps/wms/#{@map.id}")
  ...
  def update_year
    @map.update_attributes(params[:map].permit(:issue_year))
    render :json => {:message => t('.message') + @map.issue_year.to_s }
  end
