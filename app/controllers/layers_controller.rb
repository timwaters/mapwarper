class LayersController < ApplicationController
  ...
  before_filter :check_administrator_role, :only => [:publish, :toggle_visibility, :merge, :trace, :id, :create, :new] 
  ...
  before_filter :check_if_layer_is_editable, :only => [:edit, :update, :remove_map, :update_year, :update, :destroy, :delete]
  ...
  def wms()
    begin
      ...
      map.setMetaData("wms_onlineresource",
        "#{request.scheme}://" + request.host_with_port + "/layers/wms/#{@layer.id}")
  ...
  def wms2
    ...
    begin
      ...
      # logger.info map.getProjection
      map.setMetaData("wms_onlineresource",
        "#{request.scheme}://" + request.host_with_port  + "/layers/wms2")
      unless @layer_name
